...
- `docs/releases/2.0.0-rc.1/publication-evidence-2026-05-16.md` records the
  ...
  ECC-Tools #78 harness policy-route linking, PR #1947 supply-chain
...
| Linear roadmap is detailed | Linear project status plus repo mirror | Repo mirror exists; issue creation was retried on 2026-05-12 and remains blocked by the workspace free issue limit; this May 16 sync adds ECC #1860, AgentShield #78-#89, JARVIS #13, ECC-Tools #53-#78, resolved queue/discussion counts, and a generated `operator:dashboard` prompt-to-artifact audit for recurring status updates | Needs recurring status updates after each significant merge batch |
...
1. Continue the AgentShield enterprise control-plane sequence from
   ...
   evidence paths in hosted PR comments and check-runs; and ECC-Tools PR #78
   links AgentShield fleet target paths into hosted harness owner findings. The
   next slice is richer fleet-review workflow and durable policy export.
